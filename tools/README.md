...
| `stage-card.sh` | Stages the release UF2 and `lib/` into `sdcard-pico2/`, with the checks listed in `README.md` §2. `--card E:` also diffs a real card, read-only. | A built `build-pico2/angband.uf2`; `--card` needs the SD card in the PC. | Release |
...
| `turnlog.py` | Reduces an `ANGBAND_TURN_LOG` serial capture to per-phase medians and percentiles; `--compare` diffs two captures. | A device capture, and the development workspace's `.llm/scratch/bench/` convention for stored tags. **Not part of the release.** | Bench |
